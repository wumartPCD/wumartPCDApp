...
{
    __weak BaseSlideViewController *_c;
}
...
@interface BaseSlideViewController()
...
- (id)initWithFrame:(CGRect)frame controller:(BaseSlideViewController*)controller
{
...
@interface BaseSlideViewController()<UIGestureRecognizerDelegate>
{
    SlideMenuView *_contentView;
...
- (void)prepareForSegue:(SlideViewControllerSegue *)segue sender:(id)sender
{
    NSString *identifier = segue.identifier;
    if ( [segue isKindOfClass:[SlideViewControllerSegue class]] && sender == nil )
    {
        if ( [identifier isEqualToString:SWSegueRearIdentifier] )
        {
            segue.performBlock = ^(SlideViewControllerSegue* rvc_segue, UIViewController* svc, UIViewController* dvc)
            {
...
- (void)loadView
{
    ...
    _contentView = [[SlideMenuView alloc] initWithFrame:frame controller:self];
...
- (void)_dispatchSetFrontViewPosition:(FrontViewPosition)frontViewPosition animated:(BOOL)animated
{
    ...
    __weak BaseSlideViewController *theSelf = self;
...
- (BaseSlideViewController*)revealViewController
{
    ...
    Class revealClass = [BaseSlideViewController class];
...
@implementation SlideViewControllerSegue
