...
- (void)singleTap:(UITapGestureRecognizer *)recognizer
{
    for (UIView *v in self.subviews) {
        if ([v isKindOfClass:[PopMenuView class]] && [v respondsToSelector:@selector(dismissMenu:)]) {
...
+ (instancetype) menuItem:(NSString *) title
                   ...
                   action:(SEL) action
{
    return [[PopMenuItem alloc] init:title
                               image:image
                              target:target
                              action:action];
}
             ...
             menuItems:(NSArray *)menuItems
{
    ...
    PopMenuOverlay *overlay = [[PopMenuOverlay alloc] initWithFrame:view.bounds];
        ...
        if (animated) {
            ...
            [UIView animateWithDuration:0.2
                             animations:^(void) {
                                 ...
                                 if ([self.superview isKindOfClass:[PopMenuOverlay class]])
...
- (void)performAction:(id)sender
{
    ...
    PopMenuItem *menuItem = _menuItems[button.tag];
...
- (UIView *) mkContentView
{
    ...
    UIFont *titleFont = [PopMenu titleFont];
    ...
    for (PopMenuItem *menuItem in _menuItems) {
    ...
    UIImage *selectedImage = [PopMenuView selectedImage:(CGSize){maxItemWidth, maxItemHeight + 2}];
    UIImage *gradientLine = [PopMenuView gradientLine: (CGSize){maxItemWidth - kMarginX * 4, 1}];
    ...
    int itemHeight=itemY + kMarginY * 2;
    [contentView setContentSize:CGSizeMake( maxItemWidth, itemHeight)];
    
    CGSize size = ((UIViewController *)((PopMenuItem *)[_menuItems objectAtIndex:0]).target).view.frame.size;
    
    int height = size.height>size.width ? size.width:size.height;
    height = height - 60;
    height = itemHeight>height ? height:itemHeight;
    
    contentView.frame = CGRectMake(0, 60, maxItemWidth, height);
             ...
             inContext:(CGContextRef) context
{
    ...
    UIColor *tintColor = [PopMenu tintColor];
    if (tintColor) {
...
static PopMenu *gMenu;
static UIColor *gTintColor;
static UIFont *gTitleFont;

@implementation PopMenu {
    
    PopMenuView *_menuView;
    BOOL        _observing;
}
              ...
              menuItems:(NSArray *)menuItems
{
    ...
    _menuView = [[PopMenuView alloc] init];
